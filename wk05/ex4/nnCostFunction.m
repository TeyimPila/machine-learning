...
% Add ones (biase unit) to layer 2
a2 = [ones(m, 1) a2];
z3 = a2*Theta2';
a3 = sigmoid(z3);
...
Theta1s = Theta1(:,2:end); % Exclude first row of theta1 when using reglarization
Theta2s = Theta2(:,2:end); % Exclude first row of theta2 when using reglarization
J = ((1/m) .* sum(sum(temp))) + (lambda/(2*m)).*(sum(sum(Theta1s.^2)) + sum(sum(Theta2s.^2)));
...
delta_2 = delta_2(:,2:end); %regularization
